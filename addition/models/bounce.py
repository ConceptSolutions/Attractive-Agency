...
class Bounce(models.Model):
    ...
    date = fields.Date('Accural Date', required=True, default=fields.Date.today())
    ...
    contract_id = fields.Many2one('hr.contract', related='employee_id.contract_id', domain="[('company_id', '=', company_id), ('employee_id', '=', id)]")
    type_id = fields.Many2one('bounce.type')
    note = fields.Text(string="Note")
    bounce_id = fields.Many2one('bounce', string="Addition", store=True)
    percentage = fields.Float('Percentage')
    last_value = fields.Float('Last Bounce Value', compute="get_last_value")
    # use this field to know it was taken in payslip
    confirmed = fields.Boolean(string='Confirmed', )
    state = fields.Selection([('draft', "Draft"), ('confirmed', "Confirmed")], string="State", default='draft', tracking=True)
    hide_bounce_value = fields.Boolean(default=True, compute='_show_bounce_value')

    def _show_bounce_value(self):
        has_group = self.env.user.has_group('bounce_v14_e.group_addition_value')
        if has_group:
            self.hide_bounce_value = False
        else:
            self.hide_bounce_value = True
    ...
    def get_bounce_value(self):
        for rec in self:
            ...
            if rec.type == 'hour':
                ...
                # rec.bounce_value = (rec.percentage / 100) * rec.value * rec.contract_id.hour_value
            elif rec.type == 'day':
                rec.bounce_value = rec.value * rec.contract_id.day_value
            #   rec.bounce_value = (rec.percentage / 100) * rec.value * rec.contract_id.day_value
            elif rec.type == 'fixed':
                rec.bounce_value = rec.value
            elif rec.type == 'percentage':
                if rec.type_id.value_type == 'daily':
                    rec.bounce_value = (rec.value / 100) * rec.contract_id.day_value
                else:
                    rec.bounce_value = (rec.value / 100) * rec.contract_id.month_value
            elif rec.type == 'over':
                if rec.bounce_id:
...
class BounceType(models.Model):
    ...
    # ('over', 'Over Achievement'),
    value_type = fields.Selection([('daily', 'Daily'), ('monthly', 'Monthly')], default='daily')
