        ...
        if self.is_cheque ==False:
            ...
            if not self.currency_id.is_zero(write_off_amount_currency):
                ...
                if isinstance(write_off_line_vals, list):
                    # Handle multiple write-off lines
                    for write_off_vals in write_off_line_vals:
                        line_vals_list.append({
                            'name': write_off_vals.get('name', ''),
                            'amount_currency': write_off_vals['amount_currency'],
                            'currency_id': currency_id,
                            'debit': write_off_vals['balance'] if write_off_vals['balance'] > 0.0 else 0.0,
                            'credit': -write_off_vals['balance'] if write_off_vals['balance'] < 0.0 else 0.0,
                            'partner_id': self.partner_id.id,
                            'account_id': write_off_vals.get('account_id'),
                        })
                else:
                    # Handle single write-off line
                    line_vals_list.append({
