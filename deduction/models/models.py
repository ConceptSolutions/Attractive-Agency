...
class deduction(models.Model):
    ...
    type = fields.Selection(string='deduction Based On',
                            selection=[('hour', 'Hour'),
                                       ('day', 'Days'),
                                       ('fixed', 'Fixed'),
                                       ('percentage', 'Percentage'), ], required=True, )
    ...
    hide_deduction_value = fields.Boolean(default=True, compute='_show_deduction_value')

    def _show_deduction_value(self):
        has_group = self.env.user.has_group('deduction_v14_e.group_deduction_value')
        if has_group:
            self.hide_deduction_value = False
        else:
            self.hide_deduction_value = True
    ...
    def get_deduction_value(self):
        for rec in self:
            ...
            if rec.type == 'hour':
                ...
                # rec.deduction_value = (rec.percentage / 100) * rec.value * rec.contract_id.hour_value
            elif rec.type == 'day':
                rec.deduction_value = rec.value * rec.contract_id.day_value
                # rec.deduction_value = (rec.percentage / 100) * rec.value * rec.contract_id.day_value
            elif rec.type == 'fixed':
                rec.deduction_value = rec.value
            elif rec.type == 'percentage':
                if rec.type_id.value_type == 'daily':
                    rec.deduction_value = (rec.value / 100) * rec.contract_id.day_value
                else:
                    rec.deduction_value = (rec.value / 100) * rec.contract_id.month_value
